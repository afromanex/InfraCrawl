...
def make_context(config_id: int) -> CrawlContext:
    cfg = CrawlerConfig(config_id=config_id, config_path='p', root_urls=['http://example.test'], max_depth=0, fetch_mode="http")
    return CrawlContext(cfg)
...
def test_fetch_raises_logs_and_returns_none(caplog):
    ...
    pages = MagicMock()
    executor = make_executor_with({"http_service": http, "pages_repo": pages})
    context = make_context(1)

    caplog.clear()
    body = executor.fetch_and_store("http://example.test", context)
    assert body is None
...
def test_storage_failure_logs_and_returns_none(caplog):
    ...
    caplog.clear()
    body = executor.fetch_and_store("http://example.test/page", context)
    assert body is None
...
def test_non_200_status_is_logged_and_body_returned(caplog):
    ...
    caplog.clear()
    body = executor.fetch_and_store("http://example.test/fail", context)
