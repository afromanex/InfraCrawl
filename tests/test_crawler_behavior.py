...
def crawler_with_mocks():
    ...
    return Crawler(executor=executor), pages_repo, links_repo, fetcher, fetcher_factory, content_review_service, crawl_policy


def test_crawl_respects_max_depth(crawler_with_mocks):
    crawler, pages_repo, links_repo, fetcher, fetcher_factory, content_review_service, crawl_policy = crawler_with_mocks
...
def test_crawl_inserts_links(crawler_with_mocks):
    ...
    crawler._executor.link_processor._same_host = MagicMock(return_value=True)
    cfg = CrawlerConfig(config_id=None, config_path='p', root_urls=['http://example.com'], max_depth=1, fetch_mode="http")
    crawler.crawl(cfg)
